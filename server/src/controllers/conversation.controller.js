...
// role: 0=owner(superadmin) / 1=admin / 2=agent / 3=CRM Agent — الاتنين الأخيرين
// "طبقة إيجنت" (agent-tier) بالنسبة لرسايل أتمتة "ما بعد الحل" (نفس المنطق
// المستخدم في middlewares/admin.js لصلاحية admin)
function isAgentTierRole(role) {
...
async function getConversationMessages(req, res) {
  ...
  if (isAgentTierRole(req.user?.role)) {
    messages = messages.filter((m) => !m.is_post_resolve);
  }
